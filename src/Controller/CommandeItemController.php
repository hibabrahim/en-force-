...
class CommandeItemController extends AbstractController
{
     ...
     */
    public function indexbycommandefront(CommandeItemRepository $commandeItemRepository, LivraisonRepository $livraison, int $idC): Response
    {
        return $this->render('commande_item/mesItems.twig.html', [
            'items' => $commandeItemRepository->findByCommande($idC),
            'livree' => $livraison->findOneBy(['commande' => $idC]),
        ]);
    }
 ...
 /**
     * @Route("/news/{idC}", name="app_commande_item_news", methods={"GET", "POST"})
     */
    public function news($idC, Request $request, CommandeItemRepository $commandeItemRepository, \App\Repository\CommandeRepository $commandeRepository): Response
{    
        $commandeItem = new CommandeItem();
        $form = $this->createForm(CommandeItemType::class, $commandeItem);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_commande_item_index_commande_front', ['idC' => $idC], Response::HTTP_SEE_OTHER);
        }
        
        return $this->renderForm('commande_item/news.html.twig', [
            'commande_item' => $commandeItem,
            'form' => $form,
        ]);
     ...
     */
    public function edit($idC, Request $request, $id, CommandeItemRepository $commandeItemRepository): Response
    {   $commandeItem=$commandeItemRepository->find($id);
        $form = $this->createForm(CommandeItemType::class, $commandeItem);
        $form->handleRequest($request);
     ...
     */
    public function delete($idC, Request $request, $id, CommandeItemRepository $commandeItemRepository): Response
    {     $commandeItem=$commandeItemRepository->find($id);
